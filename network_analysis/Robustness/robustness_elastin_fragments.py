...
def targeted_attack(graph, target_nodes):
    ...
    return largest_component_sizes, efficiency_list, num_components_list
...
if len(G) > 0:
    largest_sizes_mcc, efficiency_mcc, num_components_mcc = targeted_attack(G, important_nodes)
    ...
    plt.subplot(1, 2, 2)
    plt.plot(x, num_components_mcc, label="Number of Components", color='violet')
    plt.xlabel("Number of Nodes Removed")
    plt.ylabel("Number of Components")
    plt.title("Number of Components During Attack")
    plt.legend()
